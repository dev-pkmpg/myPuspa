    ...
    <div class="bg-gray-50 border border-gray-200 rounded-lg p-4 mb-6">
        ...
        <div class="grid grid-cols-2 gap-4">
            <div class="col-span-2">
                <label class="block text-xs font-medium text-gray-600 mb-1">Nama Lokasi</label>
                <input wire:model="nama_lokasi" type="text" placeholder="Contoh: Gedung A, Kantor Pusat"
                       class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                @error('nama_lokasi') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>
            <div class="col-span-2">
        ...
        <div class="flex gap-3 mt-4">
            <button @click="Swal.fire({
                        title: '{{ $editingId ? 'Perbarui' : 'Simpan' }} Lokasi?',
                        text: 'Pastikan nama lokasi sudah benar.',
                        icon: 'question',
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3 text-right space-x-3">
                        ...
                        <button @click="Swal.fire({
                                    title: 'Hapus Lokasi?',
                                    text: '&quot;{{ $lokasi->nama_lokasi }}&quot; akan dihapus permanen.',
                                    icon: 'warning',
                                    showCancelButton: true,
                                    ...
                                    cancelButtonText: 'Batal',
                                }).then((result) => { if (result.isConfirmed) $wire.delete({{ $lokasi->id }}) })"
